...
| deepseek-harness | `stdenv` (pnpm monorepo) | `src.hash`, `pnpmDeps.hash` — no git tags upstream: bump manually by pinning a new master commit (rev + version from `package.json` + hashes) |
| oh-my-pi | `stdenv` (autoPatchelf releases) | `sources.json` (regenerated by `nix run .#oh-my-pi.updateScript`, follows the latest GitHub release; prebuilt `omp` Bun-compiled binaries, glibc only) |
